...
{
	double next_pos, predict_pos, real_acc;
...
/// @param vel 当前速度
/// @return 敌人上一次加速度
static double GetEnemyLastAcc(double My_acc, double vel)
{
	static double Enemy_last_acc, last_vel, My_last_acc = 1000;
	if (My_last_acc != 1000) // 类似continue，跳过当前计算（现在没有"last_"前缀的量），目的是获得"last_"前缀的量
		Enemy_last_acc = My_last_acc + last_vel - vel;
...
void player1(struct war_info *info)
{
	...
	static double Enemy_acc,acc;

	Enemy_acc = GetEnemyLastAcc(info->player_acc, info->vel);
	...
	else if (YOU_DANGER(Enemy_acc, info->zones[2]) && info->ts_left > 2 && EnemyType == 1) // 针对进攻型 他危险时松手
	{
	...
	{ // 针对进攻型
		...
		for (acc=0; I_SAFE(acc, info->zones[2] - 15); acc += 0.01) // 促使对手增大拉力减15是为了防止风吹飞我
			;
	...
	else if (info->ts_left == 2) // 针对所有
	{
		...
		{
			info->player_acc = 50 - info->wind - 2 - info->vel; // 不会有人从头到尾都不发力吧。
		}
		return;
